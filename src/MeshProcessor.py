...
class MeshProcessor:
    ...
    def __init__(self, vertices, faces, ttype, source_dir=None,from_file = False,
                ...
                compute_splu=True, load_splu=False,fix_mesh=False):
        '''
        :param vertices:
        :param faces:
        ...
        if fix_mesh:
            cleaning_data = igl.remove_duplicates(self.vertices,self.faces,1e-7)
            self.vertices = cleaning_data[0]
            self.faces = cleaning_data[1]

            cleaning_data = igl.resolve_duplicated_faces(self.faces)
            self.faces = cleaning_data[0]

            cleaning_data = igl.remove_unreferenced(self.vertices,self.faces)
            self.vertices = cleaning_data[0]
            self.faces = cleaning_data[1]
        #print(self.faces.shape)
        self.normals =  igl.per_vertex_normals(self.vertices, self.faces)
        self.face_normals = igl.per_face_normals(self.vertices,self.faces,np.ones((1,3)))
    ...
    @staticmethod
    def meshprocessor_from_file(fname, ttype, cpuonly=False, load_wks_samples=False, load_wks_centroids=False,fix_mesh=False):
        if fname[-4:] == '.obj':
            V, _, _, F, _, _ = igl.read_obj(fname)
        elif fname[-4:] == '.off':
            V,F,_ = igl.read_off(fname)
        return MeshProcessor(V,F,ttype,os.path.dirname(fname),True, cpuonly=cpuonly, load_wks_samples=load_wks_samples, load_wks_centroids=load_wks_centroids, compute_splu=False,fix_mesh=fix_mesh)

    @staticmethod
    def meshprocessor_from_array(vertices, faces, ttype,source_dir=None, cpuonly=False, load_wks_samples=False, load_wks_centroids=False,fix_mesh=False):
        return MeshProcessor(vertices,faces,ttype,source_dir=source_dir, cpuonly=cpuonly, load_wks_samples=load_wks_samples, load_wks_centroids=load_wks_centroids, compute_splu=False,fix_mesh=fix_mesh)

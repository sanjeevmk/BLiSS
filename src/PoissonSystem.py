...
class SparseMat:
    ...
    def to_coo(self):
        vals = self.vals.cpu().detach().numpy()
        inds = self.inds.cpu().detach().numpy()
        return coo_matrix((vals, (inds[0,:], inds[1,:])), shape = (self.n, self.m))
...
class PoissonSolver:
    ...
    def solve_poisson(self,jacobians): 
        ...
        #print(jacobians.transpose(2,3).reshape(jacobians.shape[0], -1, 3, 1).squeeze(3).size())
...
if __name__ == "__main__":
    ...
    obj_path = sys.argv[1]
    import trimesh
    _mesh = trimesh.load(obj_path,process=False)
    poisson_matrices = poisson_system_matrices_from_mesh(np.array(_mesh.vertices),np.array(_mesh.faces))
    poisson_solver = poisson_matrices.create_poisson_solver().to('cuda')
    test_inp = torch.randn(1,len(_mesh.faces),3,3).float().cuda()
    solution = poisson_solver.solve_poisson(test_inp)
    print(solution.size())
